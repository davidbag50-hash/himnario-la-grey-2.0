...
const defaultHomeCardOrder=[...shell.querySelectorAll('.exact-card-grid > .exact-card')].map(card=>card.dataset.exactOpen).filter(Boolean);
function preferredHomeResource(){const p=savedProfile();if(!p||p.id==='visitor')return null;const roles=Array.isArray(p.musicRoles)?p.musicRoles:[];if(p.instrument==='voice')return'voice';if(p.instrument==='bass'||p.instrument==='drums')return'academy';if(p.instrument==='piano'||p.instrument==='guitar'||p.instrument==='all')return'chords';if(roles.includes('voice'))return'voice';if(roles.some(role=>['bass','drums'].includes(role)))return'academy';if(roles.some(role=>['guitar','piano'].includes(role)))return'chords';return null}
function personalizeHomeCards(){const grid=shell.querySelector('.exact-card-grid');if(!grid)return;const cards=new Map([...grid.querySelectorAll(':scope > .exact-card')].map(card=>[card.dataset.exactOpen,card]));defaultHomeCardOrder.forEach(key=>{const card=cards.get(key);if(card)grid.appendChild(card)});const focus=preferredHomeResource(),card=focus?cards.get(focus):null;if(card)grid.prepend(card)}
