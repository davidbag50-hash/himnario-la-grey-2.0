...
function preferredHomeResource(){const p=savedProfile();if(!p||p.id==='visitor')return null;const roles=Array.isArray(p.musicRoles)?p.musicRoles:[];if(p.instrument==='voice')return'voice';if(p.instrument==='piano'||p.instrument==='guitar'||p.instrument==='all')return'chords';if(roles.includes('voice'))return'voice';if(roles.some(role=>['guitar','piano'].includes(role)))return'chords';return null}
