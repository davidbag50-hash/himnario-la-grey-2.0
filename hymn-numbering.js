...
function setSongSeo(s){if(!s)return;const url=songUrl(s),label=songLabel(s),desc=songDescription(s),title=`${label} — ${tx('Acordes','Chords')} | La Grey`;document.title=title;canonical(url);setNamed('description',desc);setProp('og:locale',lang()==='en'?'en_US':'es_PA');setProp('og:title',title);setProp('og:description',desc);setProp('og:url',url);setNamed('twitter:title',title);setNamed('twitter:description',desc);siteStructured();structured(s,url,desc)}
