...
self.addEventListener('fetch',event=>{
  ...
  // No intervenir en Analytics, APIs u otros recursos externos.
  if(url.origin!==self.location.origin)return;

  // El collage de arranque debe aparecer inmediatamente incluso sin red.
  ...
  // Solo una navegación HTML puede caer a index.html cuando no hay red.
  ...
  // JS, CSS, imágenes y demás GET internos: red primero, caché del mismo recurso después.
  // ignoreSearch permite que un recurso precargado sin ?v= coincida con su URL versionada.
  event.respondWith((async()=>{
    try{
      const response=await fetch(event.request,{cache:'no-store'});
      if(response.ok){
