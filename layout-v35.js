...
function toggleScrollPopup(){const p=$('#lg40ScrollPopup');if(!p)return;const source=$('#readerSpeed');if(source){$('#lg40ScrollSpeed').value=source.value;$('#lg40ScrollValue').textContent=source.value+'%'}p.classList.toggle('open')}
...
function closeSongTools(){const drawer=$('#lg35SongToolsDrawer');drawer?.classList.remove('open');$('#lg40ScrollPopup')?.classList.remove('open')}
function collectSongTools(){ensureSongDrawer();const detail=$('#detail');if(!detail)return;const has=!detail.classList.contains('hidden');document.body.classList.toggle('lg35-song-open',has);if(!has)closeSongTools();syncSongToolbar()}
function syncSongToolbar(){const hymn=!!$('#hymnTransposeV2');['#lg38ToneDown','#lg38ToneUp','#lg38ToneReset','#lg38ToneSave'].forEach(s=>$(s)?.classList.toggle('hidden-tool',!hymn));const fav=$('#favBtn')?.textContent||'';if($('#lg38Fav'))$('#lg38Fav').textContent=fav.includes('★')?'♥':'♡';const play=$('#readerPlay')?.textContent||'';const running=/Pausar|Pause/.test(play);if($('#lg38Scroll'))$('#lg38Scroll').textContent=running?'⏸':'▶';if($('#lg40ScrollToggle'))$('#lg40ScrollToggle').textContent=running?'⏸':'▶';const rs=$('#readerSpeed');if(rs&&$('#lg40ScrollSpeed')&&document.activeElement!==$('#lg40ScrollSpeed')){$('#lg40ScrollSpeed').value=rs.value;$('#lg40ScrollValue').textContent=rs.value+'%'}}
