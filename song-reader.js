...
let running=false,raf=0,last=0,pinchStart=0,pinchFont=0;
const minFont=11,maxFont=30;
...
function reset(){stop();const chart=document.querySelector('#detail .chart');const y=(chart?.getBoundingClientRect().top||0)+window.scrollY-78;window.scrollTo({top:Math.max(0,y),behavior:'smooth'})}
function inject(){const head=document.querySelector('#detail .head');if(!head||$('readerTools'))return;const box=document.createElement('div');box.id='readerTools';box.className='reader-tools';box.innerHTML=`<div class="reader-tools-top"><button id="readerPlay" class="btn reader-play" type="button">▶ Auto-scroll</button><button id="readerReset" class="btn reader-reset" type="button" title="Volver al inicio de la letra">↺</button></div><div class="reader-speed-wrap"><span>🐢</span><input id="readerSpeed" class="reader-speed" type="range" min="1" max="100" step="1" value="32" aria-label="Velocidad de desplazamiento"><span id="readerSpeedValue" class="reader-speed-value">32%</span></div><p class="reader-tip">Dos dedos sobre la letra: junta para reducir · separa para ampliar. El auto-scroll sigue bajando sin tocar la pantalla.</p>`;head.appendChild(box);const saved=clamp(Number(localStorage.getItem(speedKey)||32),1,100);$('readerSpeed').value=saved;$('readerPlay').addEventListener('click',toggle);$('readerReset').addEventListener('click',reset);$('readerSpeed').addEventListener('input',updateSpeed);updateSpeed()}
function dist(t){const dx=t[0].clientX-t[1].clientX,dy=t[0].clientY-t[1].clientY;return Math.hypot(dx,dy)}
function setupPinch(){const wrap=document.querySelector('#detail .chart');const chart=$('chart');if(!wrap||!chart)return;wrap.addEventListener('touchstart',e=>{if(e.touches.length===2){stop();pinchStart=dist(e.touches);pinchFont=parseFloat(getComputedStyle(chart).fontSize)||17}},{passive:true});wrap.addEventListener('touchmove',e=>{if(e.touches.length!==2||!pinchStart)return;e.preventDefault();const scale=dist(e.touches)/pinchStart;const next=clamp(pinchFont*scale,minFont,maxFont);chart.style.fontSize=next.toFixed(1)+'px';localStorage.setItem('lagrey_font',String(Math.round(next)))},{passive:false});wrap.addEventListener('touchend',e=>{if(e.touches.length<2){pinchStart=0;pinchFont=0}},{passive:true})}
function watchViews(){const detail=$('detail');if(detail)new MutationObserver(()=>{if(detail.classList.contains('hidden'))stop()}).observe(detail,{attributes:true,attributeFilter:['class']});}
function wire(){inject();setupPinch();watchViews();document.addEventListener('visibilitychange',()=>{if(document.hidden)stop()});}
