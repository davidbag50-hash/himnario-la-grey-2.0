...
async function refreshCloudProfile(expectedSlug=null){
 await ensureCloud();const state=await window.LAGREY_CLOUD.boot();
 if(state.mode!=='ministry'||!state.ministry)return{state,profile:null};
 if(expectedSlug&&state.ministry.slug!==expectedSlug)return{state,profile:null};
 return{state,profile:await cloudProfileFromState(state)};
}
...
function openProfileModal(){const p=modalParts();if(!p)return;renderStep('choose');if(profile&&profile.id!=='visitor')setStatus(`${profile.name} · ${groupText(profile)} · ${roleText(profile)}`,true);p.modal.classList.remove('hidden')}
function saveProfile(p){profile=p;localStorage.setItem(key,JSON.stringify(p));closeProfileModal();updateUI();if(p.id!=='visitor'){const pref=preferredInstrument();const msg=pref==='piano'?tx('Piano será tu vista predeterminada de acordes.','Piano will be your default chord view.'):tx('Guitarra será tu vista predeterminada de acordes.','Guitar will be your default chord view.');showToast(lang()==='en'?`Welcome, ${p.name}! ${msg}`:`¡Bienvenido, ${p.name}! ${msg}`)}else showToast(tx('Entraste como invitado.','You entered as a guest.'));document.dispatchEvent(new CustomEvent('lagrey:profile-changed',{detail:{profile:p}}))}

async function cloudLogin(expectedSlug){
 const email=String($('profileName')?.value||'').trim(),password=String($('profilePassword')?.value||'');
 if(!email||!password){setStatus(tx('Escribe tu correo y contraseña.','Enter your email and password.'));return}
 const btn=$('profileLoginBtn');if(btn)btn.disabled=true;setStatus(tx('Conectando con La Grey Cloud…','Connecting to La Grey Cloud…'));
 try{
  await ensureCloud();await window.LAGREY_AUTH.signIn({email,password});
  const result=await refreshCloudProfile(expectedSlug||null);
  if(result.profile){setStatus(`${result.profile.name} · ${result.profile.ministryName} · ${roleLabel(result.profile.cloudRole)}`,true);setTimeout(()=>saveProfile(result.profile),220);return}
  if(expectedSlug){await window.LAGREY_AUTH.signOut();setStatus(tx('Esta cuenta no pertenece al grupo La Grey.','This account does not belong to the La Grey group.'));return}
  if(result.state?.mode==='authenticated-no-ministry'){setStatus(tx('Sesión iniciada. Ahora introduce el código de tu ministerio.','Signed in. Now enter your ministry code.'),true);setTimeout(()=>renderStep('external-code'),280);return}
  setStatus(tx('Tu cuenta no tiene un ministerio activo.','Your account has no active ministry.'));
 }catch(error){setStatus(friendlyAuthError(error))}finally{if(btn)btn.disabled=false}
}
...
function primaryAction(){if(step==='choose')return renderStep('lagrey-auth');if(step==='lagrey-auth')return cloudLogin('la-grey');if(step==='external-auth')return cloudLogin(null);if(step==='external-code')return joinExternal()}
function otherAction(){renderStep('choose')}
