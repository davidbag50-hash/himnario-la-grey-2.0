...
function styleGuitar(){
 ...
 svg.querySelectorAll('text').forEach(t=>{const txt=(t.textContent||'').trim();t.setAttribute('fill',/^[1-6]$|fr|cuerda|string/.test(txt)?'#8da9bf':'#dff4ff')});
...
function stylePiano(){
 ...
 rects.forEach((r,i)=>{
   ...
   if(fill==='white'||fill==='#eaf4fb')r.setAttribute('fill','#eaf4fb');
   else if(fill==='#9ec9e5'||fill==='#57bfff')r.setAttribute('fill','#57bfff');
   else if(fill==='#20252a'||fill==='#07101a')r.setAttribute('fill','#07101a');
   else if(fill==='#2f7faa'||fill==='#f0b83a')r.setAttribute('fill','#f0b83a');
