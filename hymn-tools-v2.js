...
function refreshLanguage(){const box=$('hymnTransposeV2');if(!box)return;const label=box.querySelector('.label');if(label)label.textContent=tx('🎼 Ajustar tono','🎼 Adjust key');if($('hymnResetV2'))$('hymnResetV2').textContent=tx('↺ Restablecer','↺ Reset');if($('hymnSaveV2'))$('hymnSaveV2').textContent=tx('★ Guardar tono predeterminado','★ Save default key');render()}
function ensureBox(){let box=$('hymnTransposeV2');if(box){refreshLanguage();return box}const head=document.querySelector('#detail .head');if(!head)return null;box=document.createElement('div');box.id='hymnTransposeV2';box.className='hymn-transpose-v2';box.innerHTML='<div class="row"><span class="label"></span><span id="hymnToneNowV2" class="tone-now"></span><button id="hymnDownV2" class="btn" type="button">−½</button><button id="hymnUpV2" class="btn" type="button">+½</button><button id="hymnResetV2" class="btn" type="button"></button><button id="hymnSaveV2" class="btn save" type="button"></button></div><div id="hymnStatusV2" class="status"></div>';const meta=head.querySelector('.meta');meta?meta.insertAdjacentElement('afterend',box):head.prepend(box);$('hymnDownV2').onclick=()=>change(-1);$('hymnUpV2').onclick=()=>change(1);$('hymnResetV2').onclick=reset;$('hymnSaveV2').onclick=saveDefault;refreshLanguage();return box}
...
function saveDefault(){if(!currentSong)return;const d=defaults();d[currentSong.id]=currentShift;saveDefaults(d);render();if($('hymnStatusV2'))$('hymnStatusV2').textContent=tx('✓ Tono guardado como predeterminado para este himno.','✓ Key saved as default for this hymn.')}
